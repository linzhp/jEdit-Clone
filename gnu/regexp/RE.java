    ...
    while (index < pLength) 
    {
                ...
                && (syntax.get(RESyntax.RE_NO_BK_BRACES) ^ unit.bk))
      {
        if (currentToken == null)
          throw new REException("{ without preceding token", REException.REG_EBRACE,index);
      ...
      else if ((unit.ch == '[') && !unit.bk)
      {
        ...
        char lastChar = 0;
        if (index == pLength)
          throw new REException("unmatched '['", REException.REG_EBRACK,index);
        ...
        if ((ch = pattern[index]) == '^') 
        {
          negative = true;
          if (++index == pLength)
            throw new REException("no end of list", REException.REG_EBRACK,index);
          ch = pattern[index];
        }
      ...
      {
/*am Without this next line, a pattern that ends with an open-paren will cause
     an ArrayIndexOutOfBoundsException.*/        
        if (index == pLength)
          throw new REException("unmatched '('",REException.REG_EPAREN,index-1);
        ...
        { // not a comment
          ...
          currentToken = new RE(String.valueOf(pattern,index,endIndex-index).toCharArray(),
                                cflags,syntax,useIndex,nextSub);
      ...
      {
        if (currentToken == null)
          throw new REException("? without preceding token",REException.REG_BADRPT,index);
   ...
   * @exception IllegalArgumentException The input text was not a String, 
   * char[], or InputStream.
   */
   ...
   * @exception IllegalArgumentException The input text was not a String, 
   * char[], StringBuffer or InputStream.
   */
   ...
   *
   * @param input The input text.
   * @param replace The replacement text, which may contain $x metacharacters 
    ...
    throws REException 
  {
    ...
    if (index == input.length) 
      throw new REException("no matching brace",REException.REG_EBRACE,index);
    ...
    if (buf.length() == 0) 
      throw new REException("bad brace construct",REException.REG_EBRACE,index);
    ...
    else if ((unit.ch == ',') && !unit.bk) 
    {
      ...
      throw new REException("invalid character in brace expression",
