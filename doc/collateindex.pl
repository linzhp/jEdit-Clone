...
die $usage if ! getopts('fgi:NpP:s:o:S:I:x');
...
$depth = 0;     # how deep are we (primary=1, secondary=2, etc.)
...
print OUT "<!-- ULINK is abused here.
      ...
      The entries below are collated sorted into correct order.  Duplicates
      may be removed in the HTML backend, but in the print backends, 
      it is impossible to suppress duplicate pages or coalesce sequences
      of pages into a range.
-->\n\n";
...
sub end_entry {
    ...
    print OUT "\n";
    print OUT "  </tertiaryie>\n" if $depth == 2;
    print OUT "  </secondaryie>\n" if $depth == 1;
    print OUT "  </primaryie>\n" if $depth == 0;
    print OUT "</indexentry>\n\n";
    $depth = 0;
}
